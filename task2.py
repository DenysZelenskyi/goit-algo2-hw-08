...
class ThrottlingRateLimiter:
    ...
    def can_send_message(self, user_id: str, now: float | None = None) -> bool:
        now = time.monotonic() if now is None else now
        last = self.user_last_message_time.get(user_id, -1e18)
        return (now - last) >= self.min_interval

    def record_message(self, user_id: str) -> bool:
        now = time.monotonic()
        if not self.can_send_message(user_id, now):
            return False
